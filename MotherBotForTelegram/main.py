...
def template1(message):
    bot.send_message(message.chat.id, 'Отлично! Вот, что мне удалось найти 👇\nОтправляю видео, не переключайтесь.')
    bot.send_message(message.chat.id, '💫')
    vid_1 = open("template1.MP4", "rb")
    bot.send_animation(message.chat.id, vid_1)
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("Ссылка на код программы",
                                          url='https://github.com/ilandroxxy/TelegramBotTemplates_MotherBot/blob/main/template1.py'))
    send_mess = "В данной шаблоне, риализована динамическая клавиатура начинающаяся *от дня сегодняшнего и на 6 дней вперед*.\n\n" \
                "Далее клиенту предлагается выбрать время в которое ему было бы удобно прийти ко мне на запись.\n\nПосле того как клиент выбрал время, " \
                "данные с этим выбором приходят на мой личный *id Telegram*, где я получаю ссылку и данные о визите - остается, только связаться с клиентом."
    bot.send_message(message.chat.id, send_mess, reply_markup=markup, parse_mode="Markdown")
    send_mess2 = 'Более подробную информацию, о работе шаблона, можно найти в коде по средствам *#комментариев*\n\nТак же обращаю ваше внимание, ' \
                 'что работа с шаблоном намного облгечится, если пройти небольшой вступительный курс по разработке ботов используя библиотке *"PyTelegramBotAPI"*.\nИспользуйте команду /start'
    bot.send_message(message.chat.id, send_mess2, parse_mode="Markdown")
...
def mess(message):
    ...
    if get_message_bot == "ИП":
        ...
        markup.add(btn1, btn2, btn3, btn4)
        send_mess = "Отлично! В какой сфере вы планируете использовать бота 🤖?"
        bot.send_message(message.chat.id, send_mess, parse_mode='Markdown', reply_markup=markup)
    ...
    if get_message_bot == "Сфера услуг":
        ...
        markup.add(btn1, btn2)
        send_mess = "Какой функционал вы бы хотели предоставить боту? Что бот должен делать и как вести себя с клиентом?\n\nИз моих предположений - " \
                    "можно сделать *Лендинг/Визитку* или попробовать автоматизировать запись клиентов напрямую через мессенджер."
        bot.send_message(message.chat.id, send_mess, parse_mode='Markdown', reply_markup=markup)
    ...
    if get_message_bot == "Продажа курсов":
        ...
        markup.add(btn1, btn2, btn3)
        send_mess = "Какой функционал вы бы хотели предоставить боту? Что бот должен делать и как вести себя с клиентом?\n\nИз моих предположений - " \
                    "можно сделать *Лендинг/Визитку*, создать каталог курсов(товаров) или просто автоматизировать ответы на часто задаваемые вопросы по услугам."
        bot.send_message(message.chat.id, send_mess, parse_mode='Markdown', reply_markup=markup)
    ...
    if get_message_bot == "Магазин товаров":
        ...
        markup.add(btn1, btn2, btn3)
        send_mess = "Какой функционал вы бы хотели предоставить боту? Что бот должен делать и как вести себя с клиентом?\n\nИз моих предположений - " \
                    "можно автоматизировать техническу поддержку, чтобы не нагружать колл-центр, создать каталог товаров или просто автоматизировать ответы на часто задаваемые вопросы по услугам."
        bot.send_message(message.chat.id, send_mess, parse_mode='Markdown', reply_markup=markup)
    ...
    if get_message_bot == "Запись клиентов":
        bot.send_message(message.chat.id, 'Отлично! Вот, что мне удалось найти 👇\nОтправляю видео, не переключайтесь.', parse_mode="Markdown")
        bot.send_message(message.chat.id, '💫')
        vid_1 = open("template1.MP4", "rb")
        bot.send_animation(message.chat.id, vid_1)
        markup = types.InlineKeyboardMarkup()
